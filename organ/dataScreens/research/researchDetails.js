...
const ResearchDetails = (rs) => {
    const [data, setData] = useState(null)
    const [sno, setSno] = useState(1)
    const {id} = useParams()

    async function fetchData() {
        const apiUrl = 'https://pi360.net/site/api/api_research_publications_details.php?%20institute_id=mietjammu&rs='+id;
        ...
        <React.Fragment>
            {data === null ? <View style={styles.LoadingContainer}><Text style={styles.LoadingContainer.text}>Loading...</Text></View> :
                <ScrollView style={styles.container}>
                    ...
                    <Card title="Title" data={data["ResearchTitle"]}></Card>
                    {/* <Card title="IP Type" data="Utility patent"></Card> */}
                    {/* <Card title="Country of Filing" data="India"></Card> */}
                    {/* <Card title="Patent number" data={data["Patent_Number"]}></Card> */}
                    <Card title="Abstract" data={data["Abstract"]}></Card>
                    <Card title="Author(s)" data={data["Author"]}></Card>
                     <Card title="Status" data={data["Status"]}></Card> 
                    {/* <Card title="Filing Date" data={data["Filing_Date"]}></Card> */}
                    {/* <Card title="Issue Date" data={data["Issue_Date"]}></Card> */}
                    <Card title="Full Paper (Pdf)" data={<Text style={{ color: 'blue' }}
                        onPress={() => Linking.openURL("https:\/\/pi360.net\/mietjammu" + data["Paper_Path"].replace(/\//g, "/"))}>
                        Download
                    </Text>}></Card>
                    <Card title="Proof of Acceptance(pdf)" data={<Text style={{ color: 'blue' }}
                        onPress={() => Linking.openURL("https:\/\/pi360.net\/mietjammu" + data["Acceptance_Proof"].replace(/\//g, "/"))}>
                        Download
                    </Text>}></Card>
