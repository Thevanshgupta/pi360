...
const ExtensionLecturesDetails = () => {
    const [data, setData] = useState(null)
    const [sno, setSno] = useState(1)

    async function fetchData() {
        const apiUrl = 'https://pi360.net/site/api/api_staff_development_extension_lectures_details.php?institute_id=mietjammu&rs=1';
            ...
            {data === null ? <View style={styles.LoadingContainer}><Text style={styles.LoadingContainer.text}>Loading...</Text></View> :
                data === 404 ? <View style={styles.LoadingContainer}><Text style={[styles.LoadingContainer.text, { color: "red" }]}>Network Error</Text></View> :
                    <ScrollView style={styles.container}>
                         <Card title="Topic " data={data["Lecture_Detail"]["Topic"]}></Card>
                        <Card title="Faculty Name" data={data["Lecture_Detail"]["Faculty Name"]}></Card> 
                        <Card title="Venue" data={data["Lecture_Detail"]["Venue"]}></Card>
                        <Card title="Date" data={data["Lecture_Detail"]["Date"]}></Card>
                        <Card title="Proof of Acceptance(pdf)" data={<Text style={{ color: 'blue' }}
                            onPress={() => Linking.openURL("https:\/\/pi360.net\/mietjammu" + data["Lecture_Detail"]["Acceptance_Proof"].replace(/\//g, "/"))}>
                            Download
                        </Text>}></Card>
...
export default ExtensionLecturesDetails;
